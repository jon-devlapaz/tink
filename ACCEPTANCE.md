...
| `tink skillset add <url> [name-skillset]` | Install a skillset from a GitHub tree URL (with inferred or explicit name), create-only author its `skillsets/<name>.json` pin, generate a baseline root router, and mirror to `$TINK_HOME/skillsets/` |
| `tink skillset add <name>-skillset` | Install one pin-defined, revision-pinned skillset definition as a nested project tree, generate a baseline root router if missing, and mirror it to `$TINK_HOME/skillsets/` |
...
| I10 | Run `init --with-tink-skills` against an incomplete optional bundle, repair the bundle, then rerun the same command | First run fails but preserves completed setup; second run exits 0 and converges to `manage-tink`, `skill-scout`, and `grill-me-with-jev` |
...
| K1C | Installed skillset with root `SKILL.md` deleted | `skill check` exits ≠ 0 citing missing router; clean `skillset refresh`/`add` restores the library router when present, otherwise regenerates a baseline; custom routers survive member-clean restore |
...
| K10 | `skillset refresh <name>-skillset` after the pinned definition changes | Stages and rename-replaces the clean project tree with best-effort rollback, then mirrors the validated result to `$TINK_HOME/skillsets/`; refuses local project modifications. On publish+rollback double failure, `publish_staged_tree` keeps the displaced tree at an explicit recovery path named in the error (`skills::tests::rollback_failure_retains_recovery_backup`) |
