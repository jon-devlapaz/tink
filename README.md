...
On a TTY, `init` asks whether to install `skill-scout` and `interrogate`
...
- Refresh only clean GitHub imports; local edits are refused. The explicit
  ...
  refused. Run this in existing project
  workspaces after a binary upgrade to update local instructions.
